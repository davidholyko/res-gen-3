...
- Full-app coverage. This spec establishes the target, tooling, and
  process; it doesn't retroactively audit pages/components that don't
  exist yet. Every future PR touching UI is expected to hold the bar
  defined here, enforced by the tooling this spec adds.
...
|---|---|---|---|---|
| 1 | `<title>` is the placeholder "Create Next App"; `description` meta is the placeholder "Generated by create next app" — neither describes this app | 2.4.2 Page Titled (A) | Violation | Fixed |
| 2 | `target="_blank"` links ("Deploy Now", "Documentation") don't warn users a new tab will open (visually or via accessible name) | 3.2.5 Change on Request (AAA, adopted here as good practice) | Minor / best-practice | Fixed |
| 3 | No focus-visible styling is defined anywhere — relies entirely on browser default outline. Works today, but there's no regression guard once custom components start overriding it | 2.4.7 Focus Visible (AA) | At-risk, not yet violated | Open — no custom components exist yet to regress; revisit when one is added |
| 4 | No skip-link / bypass mechanism | 2.4.1 Bypass Blocks (A) | Not yet applicable — single-landmark page has nothing to skip; required once nav/header exists | Open — N/A until multi-landmark nav exists |
...
### Automated (wired into the existing pipeline)

- **Lint**: `eslint-plugin-jsx-a11y` layered on top of
  `eslint-config-next` in `_frontend/eslint.config.mjs`. Next's config
  already registers the `jsx-a11y` plugin with 6 rules as warnings; we
  add the plugin's full `recommended` rule set (34 rules) as a `rules`-only
  override so it doesn't re-register the plugin (ESLint flat config
  rejects redefining a plugin under the same name). Runs via the existing
  `pnpm lint`, already gated by `ci.yml` and the Claude Code pre-commit
  hook — no new CI plumbing needed.
- **Component tests**: `axe-core` directly (not `vitest-axe` or
  `jest-axe` as originally considered — `vitest-axe` hasn't published a
  stable release in 3 years, and `jest-axe` pulls in `jest-matcher-utils`,
  reintroducing a Jest-ecosystem package into a project that just migrated
  off Jest). `axe.run(container)` + a plain `expect(violations).toEqual([])`
  needs no wrapper at all. Runs as part of `pnpm test:cov`, already gated
  by the 100% coverage requirement. Note: axe-core runs in jsdom here, not
  a real browser — jsdom doesn't implement canvas/full layout, so some
  checks (e.g. certain contrast calculations) are skipped silently. This
  is a real coverage gap, not just a caveat — see page-level scans below.
- **Page-level scans**: out of scope until a browser-automation tool
  (Playwright) is set up — tracked as an open question below, not
  implemented by this spec. This is what would close the jsdom gap above.
...
`jsx-a11y` lint violations and `axe-core` assertion failures block commits
and PRs the same way lint/test failures do today (same hook and CI paths,
no new gate was needed). The manual checklist is a PR-review step, not
CI-enforced.

## Acceptance criteria

- [x] This spec is reviewed and approved
- [x] `eslint-plugin-jsx-a11y` and `axe-core` are wired into the existing
      lint/test:cov pipeline (no new CI jobs needed)
- [x] Findings #1 and #2 from the audit above are fixed
- [x] This spec is the reference future UI work is written against — new
      components are expected to include the semantic/keyboard/contrast
      requirements above by default, not retrofitted later

...
// specs/app-ux-improvements.md. Declined-confirmation branches and other
...
test.describe('UX improvements', () => {
  ...
  test('deleting a block is gated behind a real confirmation dialog', async ({
    page,
  }) => {
    // The fixture's own `page.on('dialog', ...)` (registered before this
    // test body runs) is what actually accepts the dialog -- this listener
    // only reads its message, it must not also call accept()/dismiss(),
    // or Playwright throws "Dialog already handled!".
    let dialogMessage = '';
